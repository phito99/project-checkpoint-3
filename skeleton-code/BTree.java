...
public class BTree {
    ...
    public BTree(int t, List<Student> s){
        this.root = null;
        this.t = t;
        this.studList=s;
    }
    ...
    public BTree insert(Student student) {
        ...
        //if student is not already in database add to Student.csv
        if (!exists){
            this.studList.add(student);
            try {
...
/* 
    ...
    int recurseDelete(BTreeNode parent, BTreeNode currNode, long entry, int currNodePos) {
        ...
        if (!currNode.leaf) {
            ...
            if(!removed){
                throw new ArithmeticException("Could not find Student ID to remove");
            }
            currNode.n--;
